  ...
  function refresh(callback, showLoad) {
    ...
    return bitcoin.getStats(function(err, stats_) {
      ...
      items = stats.transactions.reduce(function(out, tx) {
        var text;
        var date = new Date(tx.time * 1000).toISOString().split('T')[0];
        if (tx.category === 'move') {
          text = '[move]'
            + ui.sep
      ...
      if (items.length) {
        items.unshift(['{bold}  ID  ', '   Date   ', '   Amount  ',
          '              Address              ', 'Confirmations{/bold}'].join(ui.sep));
      }
      tabs.transactions._.list.setItems(items.length
        ? items
      ...
      items = stats.transactions.slice(0, 3).map(function(tx) {
        ...
        return '{green-fg}' + tx.address + '{/green-fg}'
          + sep
          + (tx.category === 'send'
            ? '{red-fg}{bold}<- ' + (+tx.amount).toFixed(8)  + '{/bold}{/red-fg}'
            : '{green-fg}{bold}<- ' + (+tx.amount).toFixed(8)  + '{/bold}{/green-fg}')
