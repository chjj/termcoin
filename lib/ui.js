...
ui.start = function(stats, callback) {
  ...
  tabs.addresses._.list.on('select', function(el, index) {
    ...
    return screen._.picker.pick(function(err, option) {
      ...
      if (option === 'Show QR Code') {
        ...
        return screen._.prompt.type(text, '', function(err, msg) {
          if (err) return screen._.msg.error(err.message);
          if (label) {
  ...
  tabs.transactions._.list.on('select', function(el, index) {
    var tx = tabs.transactions._.list._.ids[index - 1];
    if (!tx) return;
    ...
    return screen._.picker.pick(function(err, option) {
      ...
      if (option === 'Details') {
        ...
        return getTransaction(tx.txid, function(err, tx) {
          ...
          // If we have a bcoin transaction, show in block explorer.
          if (tx.d && transforms.tx.type(tx.d) === 'bcoin') {
            tx = transforms.tx.bcoinToInfo(tx.d);
          ...
          if (tx.d && transforms.tx.type(tx.d) === 'libbitcoind') {
            tx = transforms.tx.libbitcoindToInfo(tx.d);
            coined.utils.hideProperty(tx, '_noRefresh', true);
            return tabs.explore._.view({
              type: 'tx',
              value: tx
      ...
      if (option === 'Block Details') {
        ...
        return getBlock(tx.blockhash, function(err, block) {
          if (err) return screen._.msg.error(err.message);
          ...
          // If we have a bcoin transaction, show in block explorer.
          if (block.d && transforms.block.type(block.d) === 'bcoin') {
            block = transforms.block.bcoinToInfo(block.d);
            coined.utils.hideProperty(block, '_noRefresh', true);
            return tabs.explore._.view({
              type: 'block',
              value: block
          ...
          // If we have a bcoin transaction, show in block explorer.
          if (block.d && transforms.block.type(block.d) === 'libbitcoind') {
            block = transforms.block.libbitcoindToInfo(block.d);
            coined.utils.hideProperty(block, '_noRefresh', true);
            return tabs.explore._.view({
              type: 'block',
              value: block
  ...
  tabs.recipients._.list = blessed.list({
  ...
  function refresh(callback, showLoad) {
    ...
    return bitcoin.getStats(function(err, stats_) {
      ...
      items = stats.transactions.reduce(function(out, tx) {
        var text;
        var time = tx.time || (Date.now() / 1000 | 0);
        ...
        if (tx.category === 'move') {
          text = '[move]'
            ...
            + ui.sep
            + '{yellow-fg}{bold}' + (+tx.amount).toFixed(8) + '{/bold}{/yellow-fg}'
            + ui.sep
          ...
          text = tx.txid.substring(0, 6)
            ...
            + ui.sep
            + (tx.category === 'send'
              ? '{red-fg}{bold}-' + (+tx.amount).toFixed(8) + '{/bold}{/red-fg}'
              : '{green-fg}{bold}+' + (+tx.amount).toFixed(8) + '{/bold}{/green-fg}')
            //+ ui.sep
            //+ '({red-fg}' + (tx.fee ? (+tx.fee).toFixed(8) : 0..toFixed(8)) + '{/red-fg})'
            + ui.sep
      ...
      tabs.transactions._.list.setItems(items.length === 0
      ...
      items.unshift('new');
      tabs.recipients._.list.setItems(items);

      // Debug
      if (termcoin.config.debug) {
        var wallets = '';
        if (termcoin.bitcoin.name === 'bcoin') {
