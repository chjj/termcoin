...
ui.start = function(stats, callback) {
  ...
  tabs.explore.on('focus', function() {
    ...
    tabs.explore.key('h', function() {
      ...
      return blockchain.getBlock(revHex(block.prevBlock), function(err, block) {
        screen._.loader.stop();
        ...
        if (!block) return;
        tabs.explore._.data.setContent(utils.inspect(block));
    ...
    tabs.explore.key('t', function() {
      ...
      delete tabs.explore._.tx;
      if (!block) return;
      screen._.picker.setItems(block.tx);
      return screen._.picker.pick(function(err, tx) {
        ...
        screen._.loader.load('Loading...');
        tabs.explore._.loading = true;
        return blockchain.getTransaction(revHex(tx), block, function(err, tx) {
          screen._.loader.stop();
          tabs.explore._.loading = false;
          if (err) {
            return tabs.explore._.data.setContent(err.stack + '');
          ...
          if (!tx) return;
          tabs.explore._.data.setContent(utils.inspect(tx));
    ...
    screen.on('resize', function() {
      var tx = tabs.explore._.tx;
      if (tx) {
        var text = blockchain._formatTX(tx, tabs.explore._.data.width);
  ...
  function revHex(str) {
    ...
    while ((i -= 2) >= 0) out += str.slice(i, i + 2);
    return out;
  }
  ...
  screen.on('element keypress', function(el, ch, key) {
    ...
    if (el === _.question
        ...
        || el === _.fm
        || el === _.picker
        || (el === _.tabs.explore && _.tabs.explore._.tx)) {
      return;
    }
