...
transforms.block.libbitcoindToInfo = function(block) {
  ...
  blockn.main_chain = true;
  blockn.est_tx_volume = block.tx.reduce(function(out, tx) {
    return out + tx.vout.reduce(function(out, vout) {
      return out + vout.value;
    }, 0);
  }, 0);
  blockn.fee = block.fee || -1;
...
transforms.tx.libbitcoindToInfo = function(tx, block) {
  ...
  function getInputValue(input) {
    if (!input.prev_tx) return 0;
    var tx = input.prev_tx;
    var output = tx.vout[input.n];
    if (!output) return 0;
    return output.value.toString(10);
  }

  txn.inputs = tx.vin.map(function(input, i) {
    if (+input.block === 0) return {};
    var addr;
    if (input.coinbase) {
      addr = 'coinbase:' + input.coinbase;
    } else {
      var ops = coined.bcoin.script.decode(tx.vin[0].scriptSig.hex);
      var pubkey = Array.prototype.slice.call(new Buffer(tx.vin[0].scriptSig.hex, 'hex')).slice(-65);
      pubkey = coined.utils.ripesha(pubkey);
      addr = coined.bcoin.wallet.hash2addr(pubkey);
      addr = JSON.stringify(ops);
    }
    return {
      prev_out: {
        n: input.n || 0,
        value: input.prev_out ? input.prev_out.value : 0,
        addr: addr,
        tx_index: input.index || 0,
        // type: 0
        type: input.type || 0
      },
      script: input.scriptSig.asm
    };
  });
  ...
  txn.confirmations = block ? block.confirmations : -1;
...
transforms.type = function(data) {
  ...
  if (data.chainwork || data.txid || addr.tx) {
    return 'libbitcoind';
  }
  if (data.merkleRoot
      || data.ts != null
