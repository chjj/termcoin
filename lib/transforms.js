...
transforms.tx.bcoinToBitcoin = function(tx) {
  ...
  txn.blockindex = 0;
  txn.blocktime = ptx.ts || 0;
  txn.txid = ptx.rhash;
  txn.walletconflicts = [];
  txn.time = ptx.ts || 0;
  txn.timereceived = ptx.ts || 0;
...
transforms.block.infoToBcoin = function(block) {
  ...
  blockn.prevBlock = bcoin.utils.revHex(block.prev_block);
  blockn.merkleRoot = block.mrkl_root;
  blockn.totalTX = block.n_tx;
  blockn.bits = block.bits;
  blockn.nonce = block.nonce;
  blockn.version = block.ver;
  blockn._size = block.size;
  blockn.ts = block.time || 0;
  blockn.hashes = [];
  blockn.flags = [];
  blockn.tx = [];

  blockn.relayedBy = block.relayed_by || '0.0.0.0';
  blockn._height = block.height;
  blockn._nextBlock = block.next_block
    ? bcoin.utils.revHex(block.next_block)
    : bcoin.utils.toHex(bcoin.protocol.constants.zeroHash);

  blockn.nextBlock = blockn._nextBlock;
  blockn.height = blockn._height;

  blockn.network = true;

  blockn.txs = (block.tx || []).map(function(tx) {
    return transforms.tx.infoToBcoin(tx, blockn);
  });

  coined.utils.hideProperty(blockn, 'o', block);

  // blockn = bcoin.block(blockn);
  blockn.subtype = 'block';

  // Extras:
  blockn.fee = new bn(block.fee || 0);
  blockn.mainChain = block.main_chain || false;
  blockn.estTxVolume = block.ext_tx_volume || 0;
  blockn.difficulty = block.difficulty || 0;
  blockn.receivedTime = block.received_time || 0;
  blockn.reward = new bn(block.reward || 0);

  if (blockn.reward.cmpn(0) === 0) {
    blockn.reward = blockn.txs[0].outputs.reduce(function(total, output) {
      total.iadd(output.value);
      return total;
    }, new bn(0));
    if (blockn.fee.cmpn(0) === 0)
      blockn.fee = blockn.reward.sub(bcoin.block.reward(blockn.height));
  }
...
transforms.tx.infoToBcoin = function(tx, block) { 
  ...
  txn.lock = tx.lock_time || 0;
  txn.ts = tx.time || 0;
  txn.ps = 0;
  txn.block = bcoin.utils.revHex(tx.block || bcoin.utils.toHex(bcoin.protocol.constants.zeroHash));
  txn.rblock = tx.block;

  txn.relayedBy = tx.relayed_by || '0.0.0.0';
  txn._height = tx.block_height;
  txn._confirmations = tx.confirmations || 0;

  txn.height = txn._height;
  txn.confirmations = txn._confirmations;

  txn.network = true;

  function scriptify(script) {
    return bcoin.script.decode(bcoin.utils.toArray(script, 'hex'));
  }
  ...
  txn.inputs = tx.inputs.map(function(input, i) {
    ...
    if (!input.prev_out) {
      return bcoin.input({
        ...
        script: script,
        out: {
          hash: bcoin.utils.toHex(bcoin.protocol.constants.zeroHash),
          index: 0xffffffff
        }
      });
    }
    var prev_tx = { outputs: [] };
    prev_tx.outputs[input.prev_out.n] = bcoin.output({
      tx: prev_tx,
      value: new bn(input.prev_out.value),
      script: scriptify(input.prev_out.script)
    });
    return bcoin.input({
      tx: txn,
      seq: input.sequence,
      ...
      out: {
        tx: prev_tx,
        hash: bcoin.utils.toHex(bcoin.protocol.constants.oneHash),
  ...
  // txn = bcoin.tx(txn);
  // XXX Make bcoin.tx() block option more lenient
  if (block) {
    txn.block = block ? block._hash : null;
    txn.rblock = block ? block.rhash : null;
    txn.ts = block.ts || 0;
  }
...
transforms.addr.infoToBcoin = function(addr) {
  ...
  addrn.received = new bn(addr.total_received);
...
transforms.addr.webToBcoin = function(addr) {
  ...
  addrn.received = new bn(addr.btc_in);
...
transforms.addr.libbitcoindToInfo = function(addr) {
  ...
  addrn.address = addr.address;
  addrn.hash160 = new Buffer.from(bcoin.wallet.addr2hash(addr.address)).toString('hex');
...
transforms.txs.libbitcoindToBitcoin = function(txs) {
  ...
  var txns = txs.map(function(tx) {
    ...
    txn.walletconflicts = tx.walletconflicts;
    txn.time = tx.time || 0;
    txn.timereceived = tx.timereceived || 0;
    return txn;
  });
