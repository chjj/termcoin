...
blockchain._formatBlock = function(block, cols) {
  ...
  text += sep('Block', '#' + block.info.height, cols);

  text += sep('Hashes', '', cols);
  text += sep('Hash:', block._hash, cols);
  text += sep('Previous Block:', block.prevBlock, cols);
  text += sep('Next Block:', block.info.nextBlock, cols);
  text += sep('Merkle Root:', block.merkleRoot, cols);

  text += sep('Summary:', '', cols);
  text += sep('Transactions:', block.totalTX, cols);
  text += sep('Output Total:', block.info.outputTotal, cols);
  text += sep('Est. TX Volume:', block.info.estTXVolume, cols);
  text += sep('TX Fees:', block.info.txFees, cols);
  text += sep('Height:', block.height, cols);
  text += sep('Timestamp:', new Date(block.ts).toISOString(), cols);
  text += sep('Received Time:', block.info.receivedTime, cols);
  text += sep('Relayed By:', block.info.relayedBy, cols);
  text += sep('Difficulty:', block.info.difficulty, cols);
  text += sep('Bits:', block.bits, cols);
  text += sep('Size:', block.info.size, cols);
  text += sep('Version:', block.version, cols);
  text += sep('Nonce:', block.nonce, cols);
  text += sep('Reward:', block.txs[0].outputs.reduce(function(total, output) {
    return total.iadd(output.value);
  }, new bn(0)), cols);
...
blockchain._formatTX = function(tx) {
  ;
};
...
function sep(left, right, cols) {
  ...
  var rlen = (right + '').replace(/{[^}]+}/g, '').length;
  var width = llen + rlen;
  var remain = cols - width;
  var sp = Array(remain + 1 - 2).join(' ');
