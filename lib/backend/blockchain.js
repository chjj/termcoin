...
blockchain.getBlock = function(id, callback, last) {
  ...
  return this.request(['rawblock', id], function(err, block) {
    ...
    //if (last) return callback(null, block);
...
blockchain._formatAddr = function(addr, cols) {
  ...
  text += sep('Total Received:', addr.total_received, cols);
  text += sep('Total Sent:', addr.total_sent, cols);
  text += sep('Final Balance:', addr.final_balance, cols);
  ...
  addr.txs.forEach(function(tx) {
    ...
    var received = !!tx.out.filter(function(input) {
      var prev = input.prev_out;
      if (!prev) return;
      return prev.addr === addr.address;
    })[0];
