...
blockchain._formatAddr = function(addr, cols) {
  ...
  text += sep('Address', '{/green-fg}{bold}' + addr.address + '{/bold}{green-fg}', cols);
...
blockchain._formatTX = function(tx, block, cols) {
  ...
  tx.input_total = coined.utils.toBTC(tx.input_total);
  ...
  */
  ...
  text += sep('Transaction:', '{/green-fg}{bold}' + tx.hash + '{/bold}{green-fg}', cols);
...
/*
  block.fee = coined.utils.toBTC(new bn(block.fee));

  block.reward = block.tx[0].out.reduce(function(total, output) {
    return total.iadd(new bn(output.value));
  }, new bn(0));
  block.reward = coined.utils.toBTC(block.reward);
  ...
  block.output_total = coined.utils.toBTC(block.output_total);
*/
  ...
  text += sep('Block', '{/green-fg}{bold}#' + block.height + suffix + '{/bold}{green-fg}', cols);
  text += sep('', '(press {yellow-fg}`t`{/yellow-fg} to select from transactions)', cols);
