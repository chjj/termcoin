...
bitcoind.getTransaction = function(id, blockhash, callback) {
  ...
  return bitcoindjs.getTransaction(D(id), blockhash, D(function(err, tx) {
...
bitcoind.rescan = function(options, callback) {
  debug.log('rescan');
  return bitcoindjs.wallet.rescan(D(options), D(function(err, result) {
    if (err) {
      debug.error(err);
      return callback(err);
    }
...
bitcoind.getBestBlock = function(callback) {
  debug.log('getBestBlock');
  var hash = bitcoindjs.getBestBlock();
  return bitcoindjs.getBlock(hash, callback);
};

bitcoind.getChainHeight = function() {
  debug.log('getChainHeight');
  return bitcoindjs.getChainHeight();
};

bitcoind.__defineGetter__('chainHeight', function() {
  return bitcoind.getChainHeight();
});

bitcoind.getBlockByTxid =
bitcoind.getBlockByTx = function(txid, callback) {
  debug.log('getBlockByTx');
...
bitcoind.getBlocksByDate =
bitcoind.getBlocksByTime = function(options, callback) {
  debug.log('getBlocksByTime');
...
bitcoind.getLastFileIndex = function() {
  debug.log('getLastFileIndex');
