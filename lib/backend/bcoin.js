...
bitcoin.getTransaction = function(id, callback) {
  ...
  var tx = self._transformTX(account.tx._all[id]);
...
bitcoin.getBlock = function(id, callback) {
  ...
  return coin.findBlock(id, function(err, block) {
    block = self._transformBlock(block);
...
bitcoin._transformTX = function(tx) {
  var self = this;
  var coin = this.coin;
  ...
  txn.details = (function() {
    ...
    txn.confirmations = self._getConfirmations(ptx.block);
...
bitcoin._transformTXList = function(txs) {
  var self = this;
  var coin = this.coin;
  var ptxs = txs.map(function(ptx) {
    var txn = {};
    coined.utils.hideProperty(txn, 'd', ptx);
    var tx = self._transformTX(ptx);
...
bitcoin._transformBlock = function(block) {
  var self = this;
  var coin = this.coin;
  var blk = {};
  var pblock = this.coin.parseBlock(block);
  blk.hash = pblock.rhash;
  blk.confirmations = pblock.confirmations || this._getConfirmations(pblock._hash);
...
bitcoin._transformTXN = function(txn) {
  var self = this;
  var coin = this.coin;
  var tx = {};
  return tx;
...
bitcoin._transformTXNList = function(txns) {
  var self = this;
  var coin = this.coin;
  var txs = [];
  return txs;
...
bitcoin._transformBlk = function(blk) {
  var self = this;
  var coin = this.coin;
  var block = {};
  return block;
