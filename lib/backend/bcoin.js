...
bitcoin.getProgress = function(callback) {
  ...
  var genesis = {
    ...
    previousblock: bcoin.utils.revHex(bcoin.utils.toHex(gen.prevBlock))
  };
...
bitcoin.txData = function(tx) {
  ...
  if (inputs) {
    ...
    value = inputs.reduce(function(out, input) {
      return out.add(input.value);
    }, new bn(0));
  } else if (outputs) {
    addrs = outputs.reduce(function(out, output) {
      out.push(output.addr);
      return out;
    }, []);
...
bitcoin.send = function(address, amount, callback) {
  var self = this;
  var coin = this.coin;
  var amount = bcoin.utils.fromBTC(amount);
...
bitcoin.setTxFee = function(value, callback) {
  var self = this;
  var coin = this.coin;
  coin.setFee(bcoin.utils.fromBTC(value));
  return callback(null, true);
};
