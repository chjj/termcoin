...
bitcoin.getProgress = function(callback) {
  ...
  var gen = bcoin.protocol.network.genesis;
  var genesis = {
    __proto__: bcoin.protocol.network.genesis,
    blockindex: 0,
    time: gen.ts,
    timereceived: gen.ts,
    blocktime: gen.ts,
...
bitcoin.getTransactions = function(callback) {
  ...
  var txs = coin.accounts.reduce(function(out, account) {
    var txs = account.tx.all().reduce(function(out, tx) {
      ...
      if (!tx) return out;
      if (hashes[tx.hash('hex')]) return out;
...
bitcoin.txData = function(tx) {
  var coin = this.coin;
  var inputs = coin.ownInput(tx);
  var outputs = coin.ownOutput(tx);
  var addrs, value;

  if (inputs) {
    addrs = inputs.reduce(function(out, input) {
      out.push(bcoin.tx.getInputData(input).addr);
      return out;
    }, []);
    value = inputs.reduce(function(out, input) {
      return out.add(bcoin.tx.getInputData(input).value);
    }, new bn(0));
  } else if (outputs) {
    addrs = outputs.reduce(function(out, output) {
      out.push(bcoin.tx.getOutputData(output).addr);
      return out;
    }, []);
    value = outputs.reduce(function(out, output) {
      return out.add(output.value);
    }, new bn(0));
  } else {
    addrs = ['[unknown]'];
    value = tx.funds('out');
  ...
  return {
    addr: addrs[0],
    addrs: addrs,
    role: inputs ? 'sender' : 'recipient',
    target: inputs
      ? bcoin.tx.getOutputData(tx.outputs[0]).addr
      : bcoin.tx.getInputData(tx.inputs[0]).addr,
    value: value
  };
};
...
bitcoin.getTotalBalance = function(callback) {
  ...
  return callback(null, {
    balance: bcoin.utils.toBTC(balance.confirmed),
    unconfirmed: bcoin.utils.toBTC(balance.unconfirmed)
  });
};
  ...
  return this.getTransactions(function(err, txs) {
    ...
    return callback(null, coin.accounts.reduce(function(out, account) {
      ...
      var txids = txs.filter(function(tx) {
        ...
        return account.ownInput(tx) || account.ownOutput(tx);
      }).map(function(tx) {
        // return tx.txid;
        confirmations = Math.max(tx.confirmations, confirmations);
        return bcoin.utils.revHex(tx.hash('hex'));
      });
      out.push({
        address: address,
