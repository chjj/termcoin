...
tabs.explore._.setTitle = function(type) {
  ...
  if (addr) {
    ...
    picker.setLabel(' ' + type + ' for ' + tx.rhash.slice(0, 10) + ' ');
  } else if (block) {
    picker.setLabel(' ' + type + ' for '
      + block.rhash.replace(/^0+/g, '').slice(0, 10)
      + ' ');
  }
};
...
tabs.explore._.getURL = function(resource) {
  ...
  return 'https://blockchain.info/' + type
    + '/' + (resource.rhash || resource.address);
};
...
tabs.explore._.data.on('focus', function() {
  ...
  function refresh(callback) {
    ...
    if (displayed && displayed._noRefresh) {
      ...
      if (displayed.merkleRoot) {
        return blockchain.getBlock(displayed.rhash, function(err, block) {
      ...
      if (displayed.inputs) {
        return blockchain.getTransaction(displayed.rhash, tabs.explore._.block, function(err, tx) {
  ...
  tabs.explore._.data.key(['h', 'left'], function() {
    ...
    screen._.loader.load('Loading... (press {blue-bg}{bold} ? {/bold}{/blue-bg} for help)');
    return blockchain.getBlock(utils.revHex(block.prevBlock), function(err, block) {
  ...
  tabs.explore._.data.key('a', function() {
    ...
    function onInput(input, i) {
      if (input.type === 'coinbase' || input.type === 'unknown') {
        return addrs;
      }
      if (uniq[input.addr]) {
        return addrs;
      }
      uniq[input.addr] = true;
      bestAddrs.push(input.addr);
      addrs.push(input.addr
        + ' {red-fg}{bold}-' + bcoin.utils.toBTC(input.value)
        + '{/bold}{/red-fg}');
      return addrs;
    }

    function onOutput(output, i) {
      if (output.type === 'colored' || output.type === 'unknown') {
        return addrs;
      }
      if (uniq[output.addr]) {
        return addrs;
      ...
      addrs.push(output.addr
        + ' {green-fg}{bold}+' + bcoin.utils.toBTC(output.value)
        + '{/bold}{/green-fg}');
      return addrs;
    }
  ...
  tabs.explore._.data.key('t', function() {
    ...
    screen._.picker.setItems(txs.map(function(tx, i) {
      ...
      total = bcoin.utils.toBTC(total);
      var input = tx.inputs[0];
      return (i + 1) + '. ' + tx.rhash.slice(0, 7)
        + ' {cyan-fg}' + input.addr + '{/cyan-fg}'
        + ' {bold}{yellow-fg}' + bcoin.utils.toBTC(input.value) + '{/yellow-fg}{/bold}';
    }));
