...
tabs.explore._.data.on('focus', function() {
  ...
  tabs.explore._.data.key('a', function() {
    ...
    function onInput(input, i) {
      ...
      if (uniq[input.prev_out.addr] && input.prev_out.addr !== 'Unknown') {
        return addrs;
      }
    ...
    function onOutput(output, i) {
      ...
      if (uniq[output.addr] && output.addr !== 'Unknown') {
        return addrs;
      }
    ...
    function onBoth(inout, i) {
      var bestAddr = 'Unknown';

      if (inout.prev_out || !inout.addr) {
        var input = inout;
        if (!input.prev_out || input.prev_out.addr === 'Coinbase') {
          return addrs;
        }
        ...
        var inputAddr = input.prev_out
          ? input.prev_out.addr
          : 'Unknown';
        if (inputAddr !== 'Coinbase' && inputAddr !== 'Unknown') {
          bestAddr = inputAddr;
        }
      } else if (inout.addr) {
        var output = inout;
        ...
        var outputAddr = output.addr
          ? output.addr
          : 'Unknown';
        if (outputAddr !== 'Unknown') {
          bestAddr = outputAddr;
        }
      }

      bestAddrs.push(bestAddr);
    }
    ...
    if (addr) {
      addr.txs.forEach(function(tx) {
        ...
        tx.inputs.concat(tx.out).forEach(onBoth);
      });
    } else if (tx) {
      tx.inputs.forEach(onInput);
      tx.out.forEach(onOutput);
