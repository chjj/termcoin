...
tabs.explore._.displayBlock = function(block) {
  ...
  // XXX MOVE TO BLOCKCHAIN FUNCTIONS - SAME FOR FORMAT
...
tabs.explore._.data.on('focus', function() {
  ...
  tabs.explore._.data.key('a', function() {
    ...
    function onInput(input, i) {
      ...
      uniq[data.addr] = true;
      bestAddrs.push(data.addr);
      addrs.push(data.addr
        + ' {red-fg}{bold}-' + bcoin.utils.toBTC(output.value)
        + '{/bold}{/red-fg}');
      return addrs;
    }
  ...
  tabs.explore._.data.key('t', function() {
    ...
    screen._.picker.setItems(txs.map(function(tx, i) {
      ...
      var prev, data;
      if (+tx.inputs[0].out.hash === 0) {
        data = { addr: 'Coinbase' };
        prev = { value: new bn(0) };
      } else {
        prev = tx.inputs[0].out.tx.outputs[tx.inputs[0].out.index];
        data = bcoin.tx.getOutputData(prev);
      }
      return (i + 1) + '. ' + tx.rhash.slice(0, 7)
        + ' {cyan-fg}' + data.addr + '{/cyan-fg}'
        + ' {bold}{yellow-fg}' + bcoin.utils.toBTC(prev.value) + '{/yellow-fg}{/bold}';
    }));
