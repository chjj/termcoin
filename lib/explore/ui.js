...
tabs.explore._.setTitle = function(type) {
  ...
  //  } else if (tx) {
  //    picker.setLabel(' ' + type + ' for ' + tx.txid.slice(0, 10) + ' ');
  //  } else if (block) {
...
function onFocus() {
  //if (termcoin.config.libbitcoind) {
  //  return onFocusLibbitcoind();
  //}
  return onFocusBlockchain();
}

function onFocusLibbitcoind() {
  var lookup, data;
  ...
  function refresh(callback) {
    ...
    if (displayed && displayed._noRefresh) {
      ...
      if (displayed.vin) {
        return blockchain.getTransaction(displayed.txid, tabs.explore._.block, function(err, tx) {
  ...
  tabs.explore._.data.key(['h', 'left'], function() {
    ...
    screen._.loader.load('Loading... (press {blue-bg}{bold} ? {/bold}{/blue-bg} for help)');
    return blockchain.getBlock(block.previousblockhash, function(err, block) {
      screen._.loader.stop();
      if (err) return screen._.msg.error(err.message);
      return tabs.explore._.displayBlock(block);
    });
  ...
  tabs.explore._.data.key('a', function() {
    ...
    function onOutput(output) {
      if (uniq[output.scriptPubKey.addresses[0]]) return addrs;
      uniq[output.scriptPubKey.addresses[0]] = true;
      addrs.push(output.scriptPubKey.addresses[0]
    ...
    if (addr) {
      addr.tx.forEach(function(tx) {
        tx.vin.forEach(onInput);
        tx.vout.forEach(onOutput);
      });
    } else if (tx) {
      tx.vin.forEach(onInput);
      tx.vout.forEach(onOutput);
  ...
  tabs.explore._.data.key('t', function() {
    ...
    screen._.picker.setItems(txs.map(function(tx, i) {
      ...
      return (i + 1) + '. ' + tx.txid.slice(0, 7)
        + ' {cyan-fg}' + tx.vout[0].scriptPubKey.addresses[0] + '{/cyan-fg}'
    ...
    return screen._.picker.pick(function(err, tx) {
      ...
      var hash = txs[i].txid;
      screen._.loader.load('Loading... (press {blue-bg}{bold} ? {/bold}{/blue-bg} for help)');
      return blockchain.getTransaction(hash, block.hash, function(err, tx) {
        screen._.loader.stop();
        if (err) return screen._.msg.error(err.message);
        return tabs.explore._.displayTX(tx, block);
      });
...
function onFocusBlockchain() {
  ...
  function refresh(callback) {
    ...
    if (displayed && displayed._noRefresh) {
      ...
      // if (termcoin.config.libbitcoind) {
      //   if (display.chainwork) {
      //     display = transforms.block.libbitcoindToInfo(display);
      //   }
      //   if (display.txid) {
      //     display = transforms.tx.libbitcoindToInfo(display);
      //   }
      //   if (display.address && display.tx) {
      //     display = transforms.addr.libbitcoindToInfo(display);
      //   }
      // }
