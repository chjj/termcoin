...
blockchain.getBlock = function(id, callback) {
  ...
  // if (termcoin.config.libbitcoind) {
  //   return termcoin.bitcoin.getBlock(id, callback);
  // }
...
blockchain.getBlockHeight = function(height, callback) {
  ...
  // if (termcoin.config.libbitcoind) {
  //   return termcoin.bitcoin.getBlockHeight(height, callback);
  // }
  if (termcoin.config.localBlockchain) {
...
blockchain.getTransaction = function(id, block, callback) {
  ...
  if (termcoin.config.libbitcoind) {
    return termcoin.bitcoin.getTransaction(id, block, function(err, tx) {
      if (err) return callback(err);
      tx = transforms.tx.libbitcoindToInfo(tx);
      return callback(null, tx);
    });
  }
  // if (termcoin.config.libbitcoind) {
  //   return termcoin.bitcoin.getTransaction(id, block, callback);
  // }
  if (termcoin.config.localBlockchain) {
...
blockchain.getAddressTransactions = function(address, callback) {
  var self = this;
  if (termcoin.config.libbitcoind) {
    return termcoin.bitcoin.getAddrTransactions(id, function(err, addr) {
      if (err) return callback(err);
      addr.tx = transforms.txs.libbitcoindToInfo(addr.tx);
      return callback(null, addr);
    });
  }
  // if (termcoin.config.libbitcoind) {
  //   return termcoin.bitcoin.getAddrTransactions(address, callback);
  // }
  if (termcoin.config.localBlockchain) {
...
blockchain.getLastBlock = function(callback) {
  ...
  // if (termcoin.config.libbitcoind) {
  //   return termcoin.bitcoin.getLastBlock(callback);
  // }
  if (termcoin.config.localBlockchain) {
...
blockchain._formatAddr = function(addr, cols) {
  ...
  if (termcoin.config.libbitcoind) {
    ...
    // return blockchain._formatAddrStd(addr, cols);
  }
...
blockchain._formatTX = function(tx, block, cols) {
  ...
  if (termcoin.config.libbitcoind) {
    ...
    // block = transforms.block.libbitcoindToInfo(block);
    // return blockchain._formatTxStd(tx, block, cols);
  }
...
blockchain._formatBlock = function(block, cols) {
  ...
  if (termcoin.config.libbitcoind) {
    // block = transforms.block.libbitcoindToInfo(block);
    // return blockchain._formatBlockStd(block, cols);
  }
...
blockchain._formatAddrStd = function(addr, cols) {
  var text = '';
  ...
  var total_received = coined.utils.ntoBTC(addr.tx.reduce(function(total, tx) {
    return total + tx.vout.reduce(function(total, output) {
      return total + output.value;
    }, 0);
  }, 0));
  var total_sent = 0;
  var final_balance = total_received;
  ...
  text += '{blue-fg}Hash160{/blue-fg}'
    + '{|}' + '{green-fg}' + addr.hash160 + '{/green-fg}\n';
  text += '{blue-fg}Total Transactions:{/blue-fg}'
    + '{|}' + '{yellow-fg}' + addr.tx.length + '{/yellow-fg}\n';
  text += '{blue-fg}Total Received:{/blue-fg}'
    + '{|}' + '{green-fg}+' + total_received + '{/green-fg}\n';
  text += '{blue-fg}Total Sent:{/blue-fg}'
    + '{|}' + '{red-fg}-' + total_sent + '{/red-fg}\n';
  ...
  addr.tx.forEach(function(tx) {
    // XXX
    var sent = !!tx.vin.filter(function(input) {
      var prev = input.prev_out;
      if (!prev) return;
      return prev.scriptPubKey.addresses[0] === addr.address;
    })[0];

    var received = !!tx.vout.filter(function(output) {
      return output.scriptPubKey.addresses[0] === addr.address;
    })[0];

    // XXX
    var stotal = tx.vin.reduce(function(total, input) {
     var prev = input.prev_out;
     if (!prev) return total;
     if (prev.scriptPubKey.addresses[0] !== addr.address) return total;
     return total + +prev.value;
    }, 0);
    total = coined.utils.ntoBTC(stotal);

    var rtotal = tx.vout.reduce(function(total, output) {
      if (output.scriptPubKey.addresses[0] !== addr.address) return total;
      return total + +output.value;
    }, 0);
    rtotal = coined.utils.ntoBTC(rtotal);
    ...
    if (sent) {
      ...
      tx.vout.forEach(function(output) {
        var address = '{cyan-fg}' + output.scriptPubKey.addresses[0] + '{/cyan-fg}';
        if (output.scriptPubKey.addresses[0] === addr.address) {
          address = address.replace(/cyan/g, 'white');
        }
        text += '{|}' + '{red-fg}{bold}'
          + coined.utils.ntoBTC(output.value) + ' -> {/bold}{/red-fg}'
      ...
      tx.vin.forEach(function(input) {
        ...
        var address = '{cyan-fg}' + prev.scriptPubKey.addresses[0] + '{/cyan-fg}';
        if (prev.scriptPubKey.addresses[0] === addr.address) {
          address = address.replace(/cyan/g, 'white');
        }
        text += address
         + '{green-fg}{bold} ' + coined.utils.ntoBTC(prev.value)
...
blockchain._formatTxStd = function(tx, block, cols) {
  var text = '';

  tx.coinbase = !!tx.vin[0].coinbase;

  // XXX
  tx.input_total = tx.vin.reduce(function(total, input) {
    var prev = input.prev_out;
    if (!prev) return total;
    return total + +prev.value;
  }, 0);
  ...
  tx.output_total = tx.vout.reduce(function(total, output) {
    return total + +output.value;
  }, 0);
  tx.output_total = coined.utils.ntoBTC(tx.output_total);

  var outputs = tx.vout.slice(0, tx.vout.length > 1 ? -1 : 1);
  tx.est_total = outputs.reduce(function(total, output) {
    return total + +output.value;
  }, 0);
  tx.est_total = coined.utils.ntoBTC(tx.est_total);
  ...
  text += '{blue-fg}Size:{/blue-fg}'
    + '{|}' + '{yellow-fg}' + (new Buffer(tx.hex, 'hex')).length + '{/yellow-fg}\n';
  text += '\n';
  ...
  // XXX
  text += 'Senders:'
    + '{|}' + '{green-fg}(total: ' + (tx.vin_sz || 0) + '){/green-fg}\n';
  ...
  if (tx.coinbase) {
    ...
    text += '{|}' + '{red-fg}{bold}-' + tx.input_total + '{/bold}{/red-fg}\n';
    tx.vin.forEach(function(input) {
      var script = input.scriptSig && input.scriptSig.hex || '';
      // XXX
      var prev = input.prev_out;
      if (!prev) {
        prev = {
      ...
      var addr = prev.scriptPubKey.addresses[0];
      var value = coined.utils.ntoBTC(prev.value);
      var pscript = prev.scriptPubKey && prev.scriptPubKey.hex || '';
      var type = prev.type;
      var index = prev.n;
      text += '{cyan-fg}' + addr + '{/cyan-fg}'
        + ' {red-fg}{bold}-' + value + '{/bold}{/red-fg}\n';
  ...
  // XXX
  text += 'Recipients:'
    + '{|}' + '{green-fg}(total: ' + (tx.vout_sz || 0) + '){/green-fg}\n';
  text += '{|}'
    + '{green-fg}{bold}+' + tx.output_total + '{/bold}{/green-fg}\n';
  tx.vout.forEach(function(output) {
    var script = output.scriptPubKey.hex;
    var index = output.n;
    var addr = output.scriptPubKey.addresses[0];
    var value = coined.utils.ntoBTC(output.value);
    text += '{cyan-fg}' + addr + '{/cyan-fg}'
      + ' {green-fg}{bold}+' + value + '{/bold}{/green-fg}\n';
  });
  ...
  text += 'Input Scripts:\n';
  tx.vin.forEach(function(input) {
    var script = input.scriptSig.hex || '';
    var prev = input.prev_out;
    if (!prev) {
      prev = {
        scriptPubKey: {
...
blockchain._formatBlockStd = function(block, cols) {
  var text = '';
  ...
  if (block.tx.length) {
    block.reward = block.tx[0].vout.reduce(function(total, output) {
      return total + +output.value;
    }, 0);
    block.reward = coined.utils.ntoBTC(block.reward);
  } else {
  ...
  text += '{blue-fg}Hash:{/blue-fg}'
    + '{|}' + '{green-fg}' + block.hash + '{/green-fg}\n';
  text += '{blue-fg}Previous Block:{/blue-fg}'
    + '{|}' + '{green-fg}' + block.previousblockhash + '{/green-fg}\n';
  if (block.next_block) {
    text += '{blue-fg}Next Block:{/blue-fg}'
      + '{|}' + '{green-fg}' + block.nextblockhash + '{/green-fg}\n';
  }
  text += '{blue-fg}Merkle Root:{/blue-fg}'
    + '{|}' + '{green-fg}' + block.merkleroot + '{/green-fg}\n';
  text += '{blue-fg}Chain Work:{/blue-fg}'
    + '{|}' + '{green-fg}' + block.chainwork + '{/green-fg}\n';

  text += '\n';

  text += '{blue-fg}Summary:{/blue-fg}\n';
  text += '{blue-fg}Transactions:{/blue-fg}'
    + '{|}' + '{yellow-fg}' + block.tx.length + '{/yellow-fg}\n';
  text += '{blue-fg}Output Total:{/blue-fg}'
    + '{|}' + '{yellow-fg}' + block.output_total + '{/yellow-fg}\n';
  //text += '{blue-fg}Est. TX Volume:{/blue-fg}'
    // XXX
    + '{|}' + '{yellow-fg}' + (block.est_tx_volume || 0) + '{/yellow-fg}\n';
  text += '{blue-fg}TX Fees:{/blue-fg}' + '{|}' + '{yellow-fg}' + block.fee + '{/yellow-fg}\n';
  text += '{blue-fg}Height:{/blue-fg}' + '{|}' + '{yellow-fg}' + block.height + '{/yellow-fg}\n';
  text += '{blue-fg}Timestamp:{/blue-fg}' + '{|}' + '{green-fg}' + (new Date(block.time * 1000).toISOString()) + '{/yellow-fg}\n';
  if (block.received_time) {
    text += '{blue-fg}Received Time:{/blue-fg}' + '{|}' + '{green-fg}' + (new Date(block.timereceived * 1000).toISOString()) + '{/yellow-fg}\n';
  }
  ...
  block.tx.forEach(function(tx) {
    var iaddr = tx.vin[0].prev_out
      ? tx.vin[0].prev_out.scriptPubKey.addresses[0]
      : ['Coinbase'][0];

    var oaddr = tx.vout[0].scriptPubKey.addresses[0];

    var ival = tx.vin.reduce(function(total, input) {
      if (!input.prev_out) return total;
      return total + +input.prev_out.value;
    }, 0);
    ival = coined.utils.ntoBTC(ival);

    var oval = tx.vout.reduce(function(total, output) {
      return total + +output.value;
    }, 0);
    oval = coined.utils.ntoBTC(oval);
