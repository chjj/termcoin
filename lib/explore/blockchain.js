...
blockchain._formatAddr = function(addr, cols) {
  ...
  addr.txs.forEach(function(tx) {
    var sent = !!tx.inputs.filter(function(input) {
      ...
      var data = bcoin.tx.getOutputData(prev.tx.outputs[prev.index]);
    ...
    if (sent) {
      ...
      tx.inputs.forEach(function(input) {
        ...
        if (+prev.hash === 0) {
          ...
          data = bcoin.tx.getOutputData(prev.tx.outputs[prev.index]);
        }
...
blockchain._formatTX = function(tx, block, cols) {
  ...
  tx.coinbase = +tx.inputs[0].out.hash === 0;
  ...
  if (tx.confirmations === 0 || tx.confirmations === -1) {
    if (tx.blockHeight >= 0) {
      if (blockchain.lastBlock) {
        tx.confirmations = blockchain.lastBlock.height - tx.blockHeight + 1;
      } else {
        tx.confirmations = 1;
      }
    }
  ...
  if (termcoin.bitcoin.coin && !termcoin.config.libbitcoind) {
    ...
    if (termcoin.bitcoin.coin.blockHeight && tx.blockHeight >= 0) {
      if (tx.confirmations === 0 || tx.confirmations === -1) {
        tx.confirmations = termcoin.bitcoin.coin.blockHeight - tx.blockHeight + 1;
      }
    }
  }
  ...
  text += '{blue-fg}Block Height:{/blue-fg}'
    + '{|}' + '{yellow-fg}' + (tx.blockHeight || -1) + '{/yellow-fg}\n';
  ...
  text += '{blue-fg}Relayed By:{/blue-fg}'
    + '{|}' + '{green-fg}' + (tx.relayedBy || 'unknown') + '{/green-fg}\n';
  ...
  if (tx.coinbase) {
    ...
    tx.inputs.forEach(function(input) {
      ...
      if (+input.out.hash === 0) {
        ...
        data = bcoin.tx.getOutputData(prev);
        if (!data)
          throw new Error(JSON.stringify(prev));
      }
      var addr = data.addr;
      var value = bcoin.utils.toBTC(prev.value);
...
blockchain._formatBlock = function(block, cols) {
  ...
  text += '{blue-fg}Relayed By:{/blue-fg}' + '{|}' + '{green-fg}'
    + (block.relayedBy || 'unknown') + '{/green-fg}\n';
  if (block.difficulty) {
  ...
  block.txs.forEach(function(tx) {
    var iaddr = +tx.inputs[0].out.hash !== 0
      ? bcoin.tx.getOutputData(tx.inputs[0].out.tx.outputs[tx.inputs[0].out.index]).addr
      : 'Coinbase';

    var oaddr = bcoin.tx.getOutputData(tx.outputs[0]).addr;

    var ival = tx.inputs.reduce(function(total, input) {
      if (+input.out.hash === 0) return total;
      return total + input.out.tx.outputs[input.out.index].value.toNumber();
    }, 0);
