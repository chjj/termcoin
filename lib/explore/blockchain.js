  ...
  addr.txs.forEach(function(tx) {
    ...
    if (sent) {
      ...
      tx.inputs.forEach(function(input) {
        ...
        text += address
         + '{green-fg}{bold} ' + bcoin.utils.toBTC(data.value)
         + ' ->{/bold}{/green-fg}\n';
      });
    }
...
blockchain._formatTX = function(tx, block, cols) {
  ...
  tx.coinbase = bcoin.tx.getInputData(tx.inputs[0]).type === 'coinbase';
  // tx.coinbase = tx.inputs[0].data.type === 'coinbase';
...
*/
  ...
  tx.inputs.forEach(function(input, i) {
    var script = blockchain._formatScript(input.script || '').replace(/ /g, '\n');
